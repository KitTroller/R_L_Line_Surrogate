...
@torch.no_grad()
def rollout(model, ck, traj, W, feedback=False, device="cpu"):
    ...
    R  = dv.shape[0]
    trunk  = build_ff_trunk_input(t.expand(R, -1, 1), model.F, model.max_freq)
    ...
    for w in range(W):
        ...
        i_hat  = model(branch, trunk)[..., 0]        # (R,S)
...
def rollout_metrics(model, checkpoint, traj, W, device="cpu"):
    ...
    assisted = per_run(rollout(model, checkpoint, traj, W, feedback=True))
    predicted = per_run(rollout(model, checkpoint, traj, W, feedback=False))
    return {
        "assisted_error_rms": float(assisted).mean(),
        "predicted_error_rms": float(predicted).mean(),
        "predicted_error_max": float(predicted.max()),
        "compounding": predicted / assisted
    }
